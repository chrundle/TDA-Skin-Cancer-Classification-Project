...
boundary_files <- list.files(path="Complex-Sampled-Data-and-Plots", pattern="*_gray_boundary.csv", full.names=TRUE, recursive=FALSE)
boundary_files
# Split into three different classes
bkl_boundary_files = boundary_files[1:32]
bkl_boundary_files
mel_boundary_files = boundary_files[33:72]
mel_boundary_files
nv_boundary_files = boundary_files[72:132]
nv_boundary_files

# Get all filenames containing image dark interior data
dark_interior_files <- list.files(path="Complex-Sampled-Data-and-Plots", pattern="*_gray_dark.csv", full.names=TRUE, recursive=FALSE)
dark_interior_files
# Split into three different classes
bkl_dark_interior_files = dark_interior_files[1:32]
bkl_dark_interior_files
mel_dark_interior_files = dark_interior_files[33:72]
mel_dark_interior_files
nv_dark_interior_files = dark_interior_files[72:132]
...
# Import image boundary data from .csv file
X <- import.sampled.points(boundary_files[11], dark_interior_files[11])
X
plot(X, pch=20, col="blue", asp=1)
...
write.table(as.matrix(DV.vectors.all),file="Complex-TDA-Data/Complex-DV-Data-Vietoris-Rips.csv",row.names=FALSE, col.names=FALSE)
write.table(as.matrix(PL.vectors.all),file="Complex-TDA-Data/Complex-PL-Data-Vietoris-Rips.csv",row.names=FALSE, col.names=FALSE)
